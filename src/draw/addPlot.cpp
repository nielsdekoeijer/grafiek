...
void addElement(boost::property_tree::ptree &svg, const Plot &element) {
  ...
  for (std::size_t i = 0; i <= xnticks; i++) {
    auto x = 0.0f;
    auto y = float(hplot) + float(element.yplot) -
             denormalize<float>(float(i) / float(ynticks), 0, float(hplot));
  ...
  polyline.put("<xmlattr>.stroke", hex(Pink));
  ...
  for (std::size_t i = 0; i <= ynticks; i++) {
    auto x = 0;
    auto y = hplot + element.yplot -
             denormalize<float>(float(i) / float(ynticks), 0, hplot);
    boost::property_tree::ptree ticklongline;
    ticklongline.put("<xmlattr>.x1", x);
  ...
  ylabel.put("<xmlattr>.transform", "rotate(-90, " + std::to_string(xylabel) +
                                        ", " + std::to_string(yylabel) + ")");
